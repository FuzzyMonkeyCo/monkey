...
func (s *Resetter) exec(ctx context.Context, shower progresser.Shower, envRead map[string]string, cmds ...shellCmd) (err error) {
	...
	s.scriptsCreator.Do(func() {
			...
			if n := len(data); n > 0 {
				if x := bytes.TrimPrefix(data, []byte(comExitcode)); n != len(x) {
					if y, err := strconv.ParseInt(string(x), 10, 8); err == nil {
						...
						log.Println("[ERR] expected base10 int:", err)
					}
				}
		...
		printer := func(p func(format string, args ...interface{})) io.Writer {
			return newlinesWriter(func(data []byte) {
				if n := len(data); n > 0 {
					if bytes.HasPrefix(data, []byte(prefixSkip)) {
						return
					}
					if x := bytes.TrimPrefix(data, []byte(prefixDropPrefix)); n != len(x) {
		...
		stdout := printer(shower.Printf)
		stderr := printer(shower.Errorf)
